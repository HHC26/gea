...
func getMenuTreeMap(authorityId string) (err error, treeMap map[string][]model.SysMenu) {
	...
	err = global.GeaDb.Where("authority_id = ?", authorityId).Order("sort").Preload("Parameters").Find(&allMenus).Error
...
func AddBaseMenu(menu model.SysBaseMenu) (err error) {
	if !errors.Is(global.GeaDb.Where("name = ?", menu.Name).First(&model.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) {
		err = errors.New("存在重复name，请修改name")
	}
	err = global.GeaDb.Create(&menu).Error
	return err
}
...
func getBaseMenuTreeMap() (err error, treeMap map[string][]model.SysBaseMenu) {
	...
	err = global.GeaDb.Order("sort").Preload("Parameters").Find(&allMenus).Error
	for _, v := range allMenus {
		treeMap[v.ParentId] = append(treeMap[v.ParentId], v)
	}
	return err, treeMap
...
func GetMenuAuthority(info *request.GetAuthorityId) (err error, menus []model.SysMenu) {
	err = global.GeaDb.Where("authority_id = ? ", info.AuthorityId).Order("sort").Find(&menus).Error
	//sql := "SELECT authority_menu.keep_alive,authority_menu.default_menu,authority_menu.created_at,authority_menu.updated_at,authority_menu.deleted_at,authority_menu.menu_level,authority_menu.parent_id,authority_menu.path,authority_menu.`name`,authority_menu.hidden,authority_menu.component,authority_menu.title,authority_menu.icon,authority_menu.sort,authority_menu.menu_id,authority_menu.authority_id FROM authority_menu WHERE authority_menu.authority_id = ? ORDER BY authority_menu.sort ASC"
	//err = global.GeaDb.Raw(sql, authorityId).Scan(&menus).Error
	return err, menus
}
